$(function() {
	...
	$.ajax({  
		...
		var allTitles = JSON.parse(data.responseText);
		for (var i = 0; i < allTitles.length; i++) {
		    title = allTitles[i];
		    console.log(title);
		    var newTitle = '<tr><td><a href="/anime/' + title.id + '/' + title.safeUrl + '">' + title.englishTitle + '</a></td>';
			newTitle += '<td>' + title.dbCategory.name + '</td><td>' + title.publishYear + '</td></tr>';
